...
from threadblocker import ThreadBlocker
import json
import os
import threading
import time
...
class PersistenceManager:
	...
	def __init__(self):
		...
		self.threadBlocker = ThreadBlocker(self.lock)
	...
	def write(self, transactionID, pageID, data):
		...
		self.lsn += 1
		self._logWrite(self.lsn, transactionID, pageID, data)
		self.buffer[pageID] = { 'pid':pageID, 'lsn':self.lsn, 'taid':transactionID, 'data':data }
		if len(self.buffer) > 5:
			for page in self.buffer.values():
				self._persistPage(page)
				print('[PersistenceManager] Buffer has more then 5 elements! Persisting Page '+str(page['pid'])+'!')
			self.buffer.clear()
			self._logClear()

	def _logWrite(self, lsn, transactionID, pageID, data):
		""" Writes log data to file 'pm.log' with log-sequence-number, transaction ID and Page ID. """
		f = open('pm.log', 'a')
		logEntry = { 'lsn':lsn, 'pid':pageID, 'taid':transactionID, 'data':data }
	...
	def blockThread(self):
		self.threadBlocker.blockThread()

	def release(self):
		self.threaBlocker.release()